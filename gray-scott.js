...
function timestep() {
  ...
  if (fincrement != 0 || kincrement != 0) {
    console.log("f: " + f.toFixed(4) + "  k: " + k.toFixed(4));
  }
  ...
  for (var i = 0; i < L; ++i) {
    for (var j = 0; j < L; ++j) {
      ...
      for (var k = 0; k < seeds; ++k) {
        if (
          (i - cntrx[k]) * (i - cntrx[k]) + (j - cntry[k]) * (j - cntry[k]) <
          radii[k] * radii[k]
        ) {
          G = true;
        }
...
function initSquare() {
  ...
  context.vertexAttribPointer(vertqloc, 2, context.FLOAT, false, 0, 0);
}
